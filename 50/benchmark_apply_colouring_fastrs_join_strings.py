...
def apply_colouring_fastrs_join_strings(source_text: list[str]) -> str:
    """Apply the string join on all elements at once, no intermediate mapping"""
    return fastrs.colour_strings(source_text)


def main():
    result = apply_colouring_fastrs_join_strings(diff_strings)
